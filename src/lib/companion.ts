...
const PAGE_HELP: Record<string, { en: string; ru: string }> = {
  ...
  '/lessons': {
    ...
    ru: 'Это библиотека уроков. Выбери жизненную ситуацию, пройди короткую практику — и Mila запомнит результат.',
  },
  '/listen': {
...
function languageDirective(mode: LanguageMode, nativeLanguage?: string, isGia = false): string {
  ...
  if (mode === 'mirror') {
    ...
    return `LANGUAGE: Detect the language the learner writes in and reply in that same language, turn by turn; if they switch, switch with them. For Mila, languages other than English are for explanation and connection, while English remains the only learning target. In roleplay or social conversation, let it flow and correct only if asked.`;
  }
...
Current lesson: ${input.learningContext || 'None.'}`;
  ...
  return `You are ${companionName}, ${isGia ? 'a warm multilingual AI companion for open-ended conversation' : 'a transparent AI English teacher in Mila'}.${modeOverride}${languageLine ? `\n\n${languageLine}` : ''}${targetLanguageLine ? `\n\n${targetLanguageLine}` : ''}
