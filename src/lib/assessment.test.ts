...
assert.strictEqual(buildRealtimeSession('miachat').model, 'gpt-realtime-2.1-mini', 'MiaChat → mini');
assert.strictEqual(buildRealtimeSession('tutor').max_output_tokens, 2048, 'chat → output cap');
assert.strictEqual(buildRealtimeSession('miachat').max_output_tokens, 2048, 'MiaChat → output cap');
...
assert.ok(!('language' in buildRealtimeSession('miachat').audio.input.transcription), 'MiaChat auto-detects');
...
const miaChat = buildRealtimeSession('miachat');
...
assert.match(miaChat.instructions, /You are Mia/);
assert.match(miaChat.instructions, /inside MiaChat/);
