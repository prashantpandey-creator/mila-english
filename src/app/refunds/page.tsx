...
export const metadata: Metadata = { title: 'Payments and Refunds — Mila English', description: 'How Mila English Pro payments, access, and refund requests work.' };

export default function RefundsPage() {
  return <AppShell className="legal-page"><AppHeader backHref="/pricing" title="Payments and refunds" eyebrow="Mila English · billing" /><AppMain width="work" className="legal-page__main"><article className="legal-page__article"><p className="legal-page__kicker">PAYMENT POLICY</p><h1>No hidden renewal.</h1><p className="legal-page__meta">Effective 19 July 2026</p>
    <section><h2>What you buy</h2><p>Mila English Pro is a one-time 30-day access pass at the price displayed on the Pricing page. The period starts only after the payment processor confirms a successful payment. It does not renew automatically, so there is nothing to cancel.</p></section>
    ...
    <section><h2>Card data and receipts</h2><p>YooKassa handles payment credentials. Mila English stores only a payment identifier, amount, currency, status, and access period for reconciliation. Receipts and available payment methods are controlled by the connected merchant account.</p></section>
