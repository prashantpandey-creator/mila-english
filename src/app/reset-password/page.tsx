...
export default function ResetPasswordPage() {
  ...
  return <main className="auth-recovery"><section className="auth-recovery__card"><h1>Choose a new password</h1><p>Use at least 8 characters. Mila signs out older sessions when this password changes.</p><form onSubmit={submit}><label>New password<input type="password" minLength={8} maxLength={128} required value={password} onChange={(event) => setPassword(event.target.value)} autoComplete="new-password" /></label><button disabled={busy || !token}>{busy ? 'Saving…' : 'Save new password'}</button></form>{error ? <p className="auth-recovery__feedback" role="alert">{error}</p> : null}<a className="auth-recovery__back" href="/forgot-password">Request another link</a></section></main>;
}
