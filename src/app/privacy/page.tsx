    ...
    <AppShell className="legal-page privacy-page">
      ...
      <article className="legal-page__article">
        ...
        <section>
          ...
          <p>Mila’s companion voice room currently offers only consented <strong>Live voice</strong> to eligible preview or Pro accounts; its unfinished private/local mode is unavailable. Before Live starts, Mila explains that microphone audio and the transcript will be sent to OpenAI for real-time processing and asks for explicit consent. If you do not agree, you can continue in text chat. Pia asks separately for each page session. Audio is never sent to advertising companies.</p>
        ...
        <section className="legal-page__note">
          ...
          <p>Приватная запись на сервер Mila остаётся только в упражнениях на произношение и отдельной проверке уровня; аудио удаляется после запроса. Основная голосовая комната Mila сейчас предлагает только Live для доступного демо или Pro: аудио и текст передаются в OpenAI лишь после отдельного явного согласия. Незавершённый приватный/локальный режим основной комнаты временно недоступен; без согласия можно продолжить в текстовом чате. В отчёт об ошибке входят только введённое описание и технические данные, перечисленные выше; история чата и аудио не прикладываются. Mila не продаёт данные, не показывает рекламу и не использует межсайтовый трекинг. Учебные данные можно удалить на сайте или в приложении: <strong>Аккаунт → Удалить аккаунт и данные</strong>. Отдельная обезличенная запись об оплате может храниться для бухгалтерии и возвратов.</p>
        </section>
      </article>
      </AppMain>
    </AppShell>
