...
export const metadata: Metadata = {
  title: 'Privacy Policy — FluentMitra, Gia, and Mia',
  description: 'How FluentMitra, Gia, and Mia handle account, learning, chat, and voice data.',
}

export default function PrivacyPage() {
  return (
    <AppShell className="legal-page privacy-page">
      <AppHeader backHref="/" title="Privacy policy" eyebrow="FluentMitra · Gia · Mia" />
      ...
      <article className="legal-page__article">
        ...
        <p style={{fontSize:18,marginTop:30}}>This policy covers the related services at <strong>mila.purangpt.com</strong> (FluentMitra structured learning), <strong>gia.purangpt.com</strong> (the Gia text-and-voice companion), and <strong>mia.purangpt.com</strong> (the public travel, language, and culture experience), plus the FluentMitra iOS app and legacy miachat.purangpt.com links. Together they are called the Services below.</p>
        <p>Mia’s public Scene Studio does not require an account and keeps its most recent scene in your browser. Gia and FluentMitra have separate product experiences while using related account and service infrastructure behind the scenes.</p>
        ...
        <section>
          ...
          <ul>
            ...
            <li><strong>Payments:</strong> provider payment ID, product, amount, currency, status, and access period. FluentMitra does not receive or store full card details.</li>
          </ul>
        </section>

        <section>
          <h2>Voice data</h2>
          <p>Pronunciation tools and the private level-assessment path send a recording to FluentMitra only after you tap the microphone. Its speech service uses it to create a transcript, then deletes the recording after the request. The transcript and resulting learning feedback may remain in your learning history.</p>
          <p>The FluentMitra level assessment offers a private server voice path, a written path with no microphone, and a separate optional <strong>Live conversation</strong>. Before Live conversation starts, the Service explains that microphone audio and its transcript will be sent to OpenAI for real-time processing and asks for explicit consent. Nothing is sent to OpenAI from that path unless you agree.</p>
        ...
        <section>
          ...
          <p>Learning and account data is retained while your guest profile or account remains active. FluentMitra’s <strong>Account</strong> page can delete the shared account and its learning data. Gia’s <strong>Account</strong> page can independently delete Gia conversation history and remembered facts without touching FluentMitra learning data. These actions cannot be undone.</p>
          <p>When an account has a payment, FluentMitra may retain a detached record of the payment ID, amount, currency, status, and access period where required for accounting, fraud prevention, refunds, or legal compliance. The local record no longer has a direct relation to the deleted learning profile, but it can remain linkable through processor records where required. YooKassa and other service providers may retain data under their own legal obligations.</p>
        </section>
