...
import type { BinduState } from "./MilaBindu";
...
function compile(gl: WebGL2RenderingContext, type: number, src: string) {
  ...
  if (!gl.getShaderParameter(s, gl.COMPILE_STATUS)) {
    if (process.env.NODE_ENV !== "production") console.warn("MilaVoid shader:", gl.getShaderInfoLog(s));
    gl.deleteShader(s);
    return null;
  }
  return s;
...
export function MilaVoid({ phase, className }: { phase: BinduState; className?: string }) {
  ...
  useEffect(() => {
    ...
    if (!gl.getProgramParameter(prog, gl.LINK_STATUS)) {
      if (process.env.NODE_ENV !== "production") console.warn("MilaVoid link:", gl.getProgramInfoLog(prog));
      return;
    }
