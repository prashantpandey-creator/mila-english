...
| Mila Presence — three fictional AI avatars | **Current adult voice-room direction.** Mila, Ember, and Nocturne are a cohesive set of premium semi-realistic digital-human avatars, framed head-and-shoulders for the circular companion UI. The original voice-state meter remains around every face and changes treatment for listening, thinking, and speaking. Presence changes appearance only, never persona, data handling, or adult tone; every portrait is persistently disclosed as an AI character. Private voice remains the default, Live voice retains its explicit OpenAI-audio consent, and reference-derived campaign concepts remain unpublished. The photographic-style v1 portraits are retired from the public build and recoverable from Git history. | `5b3390c` (2026-07-23) | Deploy run [`30004675581`](https://github.com/prashantpandey-creator/mila-english/actions/runs/30004675581) green for exact SHA `5b3390c79788edd8f655913d0b0bb60fbfadd708`. Live desktop checks render all three selectable avatars with their correct versioned asset and synthetic-character label; [the 1280×720 production capture](docs/live-proof/mila-avatar-presence-5b3390c.jpg) records primary Mila. The 390×844 layout has `scrollWidth == clientWidth == 390` and no vertical overflow. All three 900×900 WebPs return 200 `image/webp` with one-year immutable caching, while both v1 JPG paths return 404. TypeScript, all 58 tests, and the production build pass. |
