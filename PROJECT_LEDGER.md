...
| Mila Presence — three fictional AI avatars + Live-only room | **Current adult voice-room direction.** Mila, Ember, and Nocturne are a cohesive set of premium semi-realistic digital-human avatars, framed head-and-shoulders for the circular companion UI. The voice-state meter remains around every face and changes treatment for listening, thinking, and speaking. The unfinished private/local companion mode is removed from the UI and Darshan runtime: eligible preview or Pro accounts can enter Live only after explicit OpenAI-audio consent, a Live failure never falls back locally, and everyone else gets an honest unavailable state pointing to text chat. Presence changes appearance only, never persona, data handling, or adult tone; every portrait remains disclosed as an AI character. Reference-derived campaign concepts remain unpublished. | Avatar set `5b3390c`; Live-only room `477fa54` (2026-07-23) | Avatar deploy [`30004675581`](https://github.com/prashantpandey-creator/mila-english/actions/runs/30004675581) and Live-only deploy [`30012245817`](https://github.com/prashantpandey-creator/mila-english/actions/runs/30012245817) are green; current voice SHA `477fa54543a4ce878b54934d044010c3b430eacd`. Live DOM contains no `Private` label and no `Voice privacy mode` control. The current consumed-preview account shows `Live voice is not available yet` and `Continue in text chat` while retaining the avatar meter; [the 620×837 production capture](docs/live-proof/mila-live-only-477fa54.jpg) records it with zero overflow. The Darshan client no longer imports the local transcription, local TTS, or chat-hook path. All three 900×900 WebPs return 200 immutable. TypeScript, all 59 tests including the Live-only launch-policy proof, and the production build pass. No microphone was activated during visual verification. |
