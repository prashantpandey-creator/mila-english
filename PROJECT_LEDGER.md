...
|------|-------|--------|-------|
| Mia place-first world language experience | **Current Mia direction.** `mia.purangpt.com` remains led by Jaipur, India and Ubud, Bali, followed by Tokyo, Lisbon, Mexico City, and Seoul, but the place-first promise is now functionally trustworthy and more companion-led. Destination matching uses normalized whole aliases instead of unsafe substrings; all curated explanatory fields are localized in Russian; draft inputs can no longer change the atmosphere of an older result; saved journeys are versioned, fully validated, and kept separately by interface language. The hero and every living-moment card open the exact promised place/situation without spending generation quota. Mia now introduces herself, plays both sides of the exchange, gives a next-step cue, and preserves a ready scene through storage/network failures. Background film can be paused and becomes poster-only for Reduced Motion or Save Data. Small-label contrast and the 720 px desktop first viewport are corrected. India/Bali retain destination-first media even when generated copy mentions a generic market or evening. No flags, faces, or country stereotypes were added. | `a1c0693` (2026-07-24) | Deploy run [`30103527510`](https://github.com/prashantpandey-creator/mila-english/actions/runs/30103527510) is green for exact SHA `a1c06934e2cd9c665b273d72bcdb5e7165419eb0`. Live hashed JS contains `READY WITH MIA`, `MIA’S NEXT STEP`, `Play their reply`, `Pause background`, `mia:last-scene`, and the corrected `hi-IN` semantic tag. The live Jaipur café deep link selects Café and renders the Hindi chai exchange; switching to Russian updates title, setting, translations, reply translation, culture cue, and mission together. The live pause control changes to Play background. At 1280×720 the primary CTA ends at y=601 with zero horizontal overflow; at 390×844 it ends at y=602 with zero overflow and the destination rail remains horizontally scrollable. The live console has no warnings/errors. [Desktop](docs/live-proof/mia-trust-companion-a1c0693.png) and [mobile](docs/live-proof/mia-trust-companion-a1c0693-mobile.png) production captures are committed. All 97 tests, TypeScript, the production build, and the focused 20-test Mia/product gate pass. |
| Gia Live synthetic companion | **Current Gia direction.** `gia.purangpt.com` is a focused dark synthetic-presence chamber with four visual looks for the same Gia identity. Eligible visitors discover and enter their one-time Live preview without a hidden query switch; consent explains OpenAI audio processing and that the preview is consumed when the session opens. The room has a trapped, keyboard-safe consent dialog, explicit connecting/listening/speaking/error states, microphone-specific recovery copy, and no PWA interruption. When Live ends or is unavailable, Gia offers a one-time session-scoped voice-to-text handoff into chat plus a clear FluentMitra Pro access path. The selected Gia appearance follows into text chat, while Gia-host account deletion remains Gia-scoped. The social card and all presence assets are public, and the root canvas/native controls stay dark before and after hydration. Short landscape, portrait, and desktop layouts preserve the portrait, actions, disclosure, and safe areas without overlap or horizontal overflow. | `341cd0f`, `7c58939`, final UI `9968624` (2026-07-24) | Deploy runs [`30102732451`](https://github.com/prashantpandey-creator/mila-english/actions/runs/30102732451), [`30103491143`](https://github.com/prashantpandey-creator/mila-english/actions/runs/30103491143), and final [`30104258371`](https://github.com/prashantpandey-creator/mila-english/actions/runs/30104258371) are green; final exact UI SHA `9968624f78212c313e18e370a86df804a1969efc`. Live hashed CSS `03743ba2532ccb72.css` contains the 520 px landscape gate and `left:34%` portrait placement; live Darshan JS contains `Your free Live preview is complete`, `Live continues with FluentMitra Pro`, `Continue in text chat`, and `handoff`. The public Gia social image returns 200 as JPEG at 95,158 bytes and the Velvet presence returns 200 as WebP at 84,198 bytes. Authenticated production QA at 1280×720 reports a 1280×720 client/scroll box, dark colour scheme, Gia product context, and no console warnings/errors; 390×844 also has zero overflow. At 568×320, portrait, invocation, and disclosure rectangles are mutually non-overlapping with zero page overflow. [The final 1280×720 production capture](docs/live-proof/gia-live-9968624.png) records the exhausted-preview handoff and Pro access state. All 97 tests, TypeScript, and the production build pass. No microphone was activated during visual verification. |
| Gia introduction, auth journey, and distinct voice | **Current Gia front door and voice direction.** `gia.purangpt.com/` is now a public, cinematic “Meet Gia” introduction instead of dropping visitors directly into Live. It presents the fictional AI presence looking forward, explains Voice and Text as two deliberate modes, links Sign in/Create account visibly, and discloses that voice begins only by choice. `/live` is the canonical protected companion URL and `/chat` remains the protected text URL; both preserve their exact destination through Gia login/guest entry, while legacy `/darshan` redirects to `/live`. The text-chat microphone now opens `/live`. Gia’s Realtime session no longer inherits the ordinary companion voice: it uses the dedicated `OPENAI_REALTIME_VOICE_GIA` override with quality default `marin`, an explicit `0.82` output speed, and prompt-level measured cadence, brief pauses, quiet mystery, restrained user-led chemistry, and anti-caricature/non-dependency boundaries. | `4f1ae50` (2026-07-25) | Deploy run [`30142620931`](https://github.com/prashantpandey-creator/mila-english/actions/runs/30142620931) is green for exact SHA `4f1ae50ea0e98ead37c8ed475627ed3d9ed33f22`; its log shows the server reset to that SHA, production build success, Realtime paid-only invariant, Gia/Mia HTTPS readiness, healthy speech services, cloud brain `OK`, and final app 200. Live anonymous HTTP verifies root 200 with `Meet Gia.`, `Enter Live voice`, `Start text chat`, and the fictional-AI/voice-choice disclosure; `/live` and `/chat` return 307 to `/login?returnTo=%2Flive` and `%2Fchat`, and `/darshan` returns 307 to `/live`. Authenticated live browser QA opens `/live` as “Live voice with Gia”; Gia login shows “Sign in for Live voice” plus a correct guest CTA and back-to-Gia link. At 1280×720 the full introduction is visible with no browser logs; at 390×844 client and scroll widths are both 375 px, the portrait is 308 px wide, the one-line H1 is 328 px wide, and both 328 px actions fit with zero horizontal overflow. All 101 tests and the production build pass. No microphone was activated during verification. |
| Gia-owned Live access journey | **Current access boundary.** The exhausted-preview action now stays inside Gia instead of opening FluentMitra pricing in another tab. `gia.purangpt.com/pricing` is a public Gia-owned Voice + Text choice with Gia-only copy, dark companion styling, a text-chat fallback, an explicit fictional-AI reminder, microphone consent, a 30-day/no-auto-renewal Live pass, and the dedicated slow/intriguing voice promise. Gia checkout uses a Gia payment description and returns to Gia’s protected `/billing/return`; paid, pending, canceled, refunded, and error states remain Gia-branded, and Gia Account now shows verified Live entitlement. The shared payment product code remains an internal entitlement detail. | `1d6d477` (2026-07-29) | Deploy run [`30407504048`](https://github.com/prashantpandey-creator/mila-english/actions/runs/30407504048) is green for exact SHA `1d6d477b64c35e85a6311a60b26878765c0e3387`. Live browser QA followed “Live access” from authenticated `/live` in the same tab to `https://gia.purangpt.com/pricing`; the DOM contains `GIA · VOICE + TEXT`, `GIA LIVE · 30 DAYS`, and no FluentMitra/Mia UI. Public pricing returns 200; the Gia billing return correctly gates to Gia login with its full return URL. The live pricing bundle contains `More time with Gia. Only when you choose it.`, `Return to Gia’s voice`, and the Gia card-data disclosure. At mobile 390×844, client and scroll widths are both 390 px. [Desktop](docs/live-proof/gia-live-access-1d6d477.png) and [mobile](docs/live-proof/gia-live-access-1d6d477-mobile.png) production captures are committed. All 101 tests, TypeScript, the focused 25-test product/payment gate, and the optimized production build pass. No microphone or payment was activated during verification. |
...
| Mila Synthetic Presence — three hybrid AI depictions + Live-only room | **Current adult voice-room direction.** Darshan is now a graphite near-future companion chamber rather than a soft portrait atelier. Mila, Ember, and Nocturne are predominantly human AI depictions with intentionally visible cranial seams, optical interfaces, engineered neck structures, individual signal colours, and a 32-segment radial voice-state meter. Mila directly adapts the adult woman-in-black reference supplied by the project owner into a non-sexualized auburn-haired synthetic companion; Ember and Nocturne adapt the project's prior fictional portraits. The selector accurately says these are AI-generated synthetic depictions and that no real person is speaking; likeness/publicity clearance for the supplied Mila reference remains the project owner's responsibility. Presence still changes appearance only, never persona, data handling, or adult tone. The unfinished private/local companion mode remains absent from both UI and Darshan runtime: eligible preview or Pro accounts can enter Live only after explicit OpenAI-audio consent, a Live failure never falls back locally, and everyone else gets the honest unavailable state pointing to text chat. | Synthetic chamber `45eae10`; Live-only behavior `477fa54` (2026-07-23) | Synthetic chamber deploy [`30018646917`](https://github.com/prashantpandey-creator/mila-english/actions/runs/30018646917) is green for exact SHA `45eae10e571e5251861056a78482e82f6486bde3`; the earlier Live-only deploy [`30012245817`](https://github.com/prashantpandey-creator/mila-english/actions/runs/30012245817) remains the behavior baseline. Live DOM resolves Mila to `/avatar/presences/mila-v3/avatar.webp`, contains no Private control, and shows `Live voice is not available yet` plus `Continue in text chat`. The three 900×900 v3 WebPs return 200 as `image/webp` at 88,976 / 144,510 / 92,186 bytes. Live hashed CSS contains `voice-chamber-ui`, `mila-portrait-presence__meter`, and `syntheticOrbit`; the 620×837 authenticated render has exact scroll/client dimensions with no overflow. [The production capture](docs/live-proof/mila-synthetic-presence-45eae10.jpg) records the final chamber. TypeScript, all 59 tests, and the production build pass. No microphone was activated during visual verification. |
| Live 3D avatar preview (`/mila-live.html`) — historical / retired | The canned-line TalkingHead prototype was removed from the public build after its stock GLB was confirmed as a non-commercial example asset. It is recoverable from Git history but must not return without separate commercial rights. | `267f08f`, retired by `42dce84` (2026-07-23) | Historical deploy run `29956513907`; current production returns 404 for both `/mila-live.html` and `/avatar/brunette.glb`. |
| Photoreal 2D idle-loop experiment — historical predecessor | The calm RunPod idle loop remains an unattached historical asset, but it is no longer the companion UI. `?face=1` now maps to the original fictional Ember Presence for compatibility, with the same synthetic-character disclosure and state treatment as the picker. | `a03b3f4`, `43d3928`, superseded by `42dce84` (2026-07-23) | Current live `/darshan?face=1` resolves to Ember; the active Presence proof is recorded above. |
...
| Android browser voice / microphone reliability — historical local-path work | **Superseded for the Darshan companion by the Live-only row above.** This release hardened the former authenticated local fallback and shared recorder against false “mic unavailable” failures, Android audio-context suspension, low-gain handset speech, late connection completion, and generic error collapse. Those reusable microphone utilities remain available to pronunciation and assessment surfaces, but Darshan no longer imports or enters this local path. **The historical release never claimed a physical Android microphone round-trip.** | `c20ca1b` (2026-07-19), Darshan path retired by `477fa54` (2026-07-23) | Historical deploy run `29669830341` green for exact SHA `c20ca1bda1bc812f66f63dc9fe4727fdd8d83b13`; current Darshan proof is recorded in the Live-only row above. |
...
- **One bright room system, one intentional exception**
  (`src/lib/routeSurface.ts`): every route still resolves to `welcome` / light,
  while `/darshan` alone carries the explicitly requested dark Synthetic
  Presence chamber inside that route. Do not spread route-specific dark focus
  rooms or device-preference palette changes elsewhere; the rest of Mila keeps
  its hierarchy through linen paper, carbon structure, eucalyptus atmosphere,
  magenta action, broad tonal gradients, typography, borders, and restrained
  shadows.
