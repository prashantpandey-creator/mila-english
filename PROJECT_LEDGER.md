...
|------|-------|--------|-------|
| Single Mila visual language | **Current source of truth for the whole site.** Marketing, auth, dashboard, chat, lessons, listening, assessment, phonetics, progress, achievements, legal pages, and companion furniture now share warm paper `#f8f4ee`, ivory `#fffaf5`, espresso ink `#241d19`, and one rose family. `#963640` is the contrast-safe light-surface action; `#e46a73` is the dark-canvas signal. Neutral near-black is limited to immersive voice rooms and the contained pricing band. Device preference and route-specific accent engines are removed. The dashboard makes **Speak with Mila** primary; the rose/ivory mascot is shared by dashboard, chat, and companion; one click enters voice mode and double-click opens the contained text panel. | `7c267ab`, merged as `25af76c` (2026-07-18) | deploy run `29652730077` green in 2m2s. Live `/`, `/start`, `/dashboard`, `/chat`, `/listen`, `/assessment`, `/darshan`, `/progress`, and `/lessons/1` all 200. Live hashed CSS contains `--mila-action:#963640`, `--guide-accent:#963640`, and `.mila-surface--app`; `/mascot/mila-mascot-rose.png` is 200 `image/png` at 534,561 bytes; the live Darshan bundle contains `Tap to talk with Mila`. Live desktop 1440x900 and mobile 390x844 screenshots verify front, dashboard, chat, voice room, and open companion panel; computed visible-colour audits found zero non-rose chromatic UI outliers and zero horizontal overflow. Companion double-click opened a fully contained panel with no action overflow; single-click entered `data-voice="1"` and the microphone-start state. Full spoken round-trip remains a physical-microphone check, not claimed from browser automation. |
