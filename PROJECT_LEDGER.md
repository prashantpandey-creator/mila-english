...
| Mila Presence — fictional visual companions | **Current adult voice-room direction.** The default remains the faceless Signal; optional Ember and Nocturne portraits are original synthetic adults generated without reference images. Presence changes appearance only, never persona, data handling, or adult tone, and every portrait is persistently disclosed as an AI character. Private voice remains the default. The front-door Live preview requires explicit OpenAI-audio consent and is atomically consumed once per authenticated registered/explicit-guest account; ongoing Live voice remains Pro. Reference-derived campaign concepts stay in ignored local storage and are not published. | `42dce84` (2026-07-23) | Deploy run [`29983667496`](https://github.com/prashantpandey-creator/mila-english/actions/runs/29983667496) green for exact SHA `42dce84ef1803cb1390876554f00460bceb52ce9`. Live desktop and 390×844 checks render Ember and Nocturne with zero horizontal overflow; the picker and disclosure-first Live dialog are present, and [the 1280×720 live capture](docs/live-proof/mila-presence-42dce84.jpg) records Ember. Live JS contains `Live preview`, `Every portrait is an original synthetic character`, and the Ember asset path; live CSS contains `presence-picker`, `mila-portrait-presence`, and `presence-ai-disclosure`. Both versioned portraits return 200 `image/jpeg` with one-year immutable caching. `/mila-live.html` and `/avatar/brunette.glb` return 404 after the non-commercial prototype retirement. Anonymous `mode=companion` now returns 401. TypeScript, all 58 tests, the atomic preview-concurrency proof, and the production build pass. |
