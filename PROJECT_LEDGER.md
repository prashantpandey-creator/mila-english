...
|------|-------|--------|-------|
| Single Mila visual language | **Current source of truth for the whole site.** Marketing, pricing, auth, dashboard, chat, lessons, listening, assessment, voice rooms, phonetics, progress, achievements, legal pages, and companion furniture now share one bright white/pink editorial house: ground `#fffdfd`, cool blush `#fff7fa` / `#fff1f5`, action rose `#b63d68`, deep berry `#8d2d50`, and ink `#2f1b24`. Every route is light; dark focus/pricing detours and cyan, emerald, gold, orange, purple, beige, and black route signals are retired. `/` uses one commissioned boho voice collage with no headphones, chair, devices, flags, landmarks, or postcard travel imagery. Listen and Assessment use the same code-native editorial choice language. The dashboard keeps **Speak with Mila** primary; one companion click enters voice mode, double-click or the visible chip opens text, and the contained panel includes a **Speak a question** control. | `24014ed`, merged as `fc777b6` (2026-07-18) | deploy run `29655772632` green in 2m38s. Eighteen live route probes returned 200 (auth-gated `/phonetics` correctly ended at `/login`). Live hashed CSS/JS contains `#b63d68`, `#fff7fa`, `mila-boho-editorial-desktop-v1.webp`, and `Not countries on postcards`; live HTML renders `English, in your own voice.` and no longer contains `Speak freely.`. Live desktop and true 390x844 mobile screenshots verify the commissioned hero, conversation dashboard, and flag-free Listen choices. A 17-route live mobile computed audit found `theme=light`, zero large dark surfaces, zero large off-palette chromatic surfaces, and zero horizontal overflow on every route. On mobile the guide does not intersect the hero CTA; its open text panel is fully contained at 359px wide with zero document overflow. Full spoken round-trip remains a physical-microphone check, not claimed from browser automation. |
...
| Uniform learning experience (historical foundation) | The `01ef5db` release established the shared lesson, quiz, word, metric, badge, action, feedback, and empty-state furniture used today. Its dark listening/assessment allocation, user-switchable themes, and dark `/start` room were superseded by the current Single Mila visual language row above. | `41121e9`, merged as `01ef5db` (2026-07-18) | Historical deploy run `29648574693` was green in 2m10s and verified the shared route furniture at that commit. |
| Front-door clips (retired) | The city-night, Manhattan, silk, Big Ben, café, headphone, chair, and device scenes are no longer loaded by the front page. `/` now renders only the commissioned white/pink Mila editorial asset; inner routes load no legacy atmosphere photography. | `24014ed`, merged as `fc777b6` (2026-07-18) | Live HTML and hashed bundle reference `/visuals/v3/mila-boho-editorial-desktop-v1.webp`; live Listen DOM contains zero flag controls and zero legacy scene images. |
...
| Cohesive inner-app theme (historical predecessor) | The `ec2e6ff` Atelier/Studio pass first unified shell, card, control, navigation, assistant, auth/legal, and learning geometry. Its colour and route-surface allocation is superseded by the current Single Mila visual language row above. | `d11ee49`, merged as `ec2e6ff` (2026-07-18) | Historical deploy `29625886226` verified the predecessor geometry; current production proof is recorded above. |
