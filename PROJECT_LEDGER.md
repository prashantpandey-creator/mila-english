...
|------|-------|--------|-------|
| Minimal Mila front door | **The public home page is now a true introduction instead of a product catalogue.** It holds one bilingual promise (“English, in your own voice”), one primary voice action, a compact listening/feedback preview, a level-check link, and a three-step method strip; the former proof grid, six-room directory, city catalogue, privacy essay, FAQ, and duplicate closing CTA are removed. The sampled bright pink `#ff82c2` anchors the voice card while the accessible `#d9006c` remains the primary action. Logged-out voice and assessment entries preserve the current explicit sign-in/register-or-guest boundary. A site-specific 1200×630 Open Graph card is live at `/og.png`. | `42424ae` (2026-07-23) | Deploy run [`29975887281`](https://github.com/prashantpandey-creator/mila-english/actions/runs/29975887281) green in 3m for exact SHA `42424ae4106ff5a50b05093b529733e0a9e31544`. Live production DOM contains the new hero, listening card, and three-step method; computed styles return `rgb(255, 130, 194)` for the card and `rgb(217, 0, 108)` for the CTA. Live hashed CSS contains `.lp-minimal__hero`, `#ff82c2`, and `#d9006c`; the live page chunk contains the new voice copy plus the `/login` → `/darshan?free=1` gate. The production title and absolute `og:image` resolve correctly, layout has no horizontal overflow at 1265×720, and the [live full-page capture](docs/live-proof/mila-front-door-minimal-42424ae.png) records the deployed result. Current-main TypeScript, all 55 tests, and the production build pass. |
